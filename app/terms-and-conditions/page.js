...
export async function generateMetadata(props, parent) {
    ...
    if (data.length > 0) {
        ...
        return {
            ...
            alternates: {
                canonical: `${process.env.siteUrl}/terms-and-conditions`,
            },
            openGraph: {
                title: seoData?.title,
                description: seoData?.description,
...
export default async function Contact() {
    ...
    return (
        <>
            ...
            <Footer         certifications={options.certifications}
 showFooterCta={false} className="mt-32" footerCtaData={options.footer_cta} contactInfo={options.contact_info} socialData={options.social_links}/>
        </>
    )
}
